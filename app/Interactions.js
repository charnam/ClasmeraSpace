...
class Interactions {
	...
	static getAvailableTargets() {
		return this.availableTargets.filter(target => target.element.checkVisibility());
	}
	...
	static getOffsetScore(rect1, rect2, direction = "up") {
		...
		if(direction == "up" || direction == "down") {
			if(rect2.right < rect1.left) {
				score += 4000;
				score += Math.abs(rect2.right - rect1.right);
			}
		...
		if(direction == "left" || direction == "right") {
			if(rect2.bottom < rect1.top) {
				score += 4000;
				score += Math.abs(rect2.bottom - rect1.bottom);
			}
